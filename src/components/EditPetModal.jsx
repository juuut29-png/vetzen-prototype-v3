...
export default function EditPetModal({ open, onClose, pet, onSaved }){
  ...
  const [historial,setHistorial]=useState(pet?.historial?.map(h => `${h.tipo}: ${h.descripcion}`).join('; ') || '')
  ...
  function save(e){
    ...
    const updated=pets.map(p=>{
      ...
      return {...p,nombre,raza,dob,peso:parseFloat(peso||0),
        edad:+c.ageYears.toFixed(1),fase:c.phase,consejoIA:c.advice,
