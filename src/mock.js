...
export function yearsFromDOB(d){ if(!d) return 0; const b=new Date(d+'T00:00:00'); return (Date.now()-b)/(1000*60*60*24*365.25) }
...
export function getWeightStatus(e,p,ra=''){
  ...
  if(e==='Perro'){
    ...
    if(p<low) return {status:'Bajo peso',advice:'Aumenta ración.'}
...
export function getPhaseAndAdviceFromDOB(e,d,p,ra=''){ const y=yearsFromDOB(d); const phase=getPhaseBySpecies(e,y); const {status:weightStatus,advice:weightAdvice}=getWeightStatus(e,p,ra); let advice=''; if(phase==='Cachorro'||phase==='Joven') advice='Vacunas y socialización.'; else if(phase==='Adulto') advice='Chequeo anual.'; else if(phase==='Senior') advice='Revisiones semestrales.'; return {ageYears:y,phase,advice,weightStatus,weightAdvice} }

export function loadPets(){
  let stored=[]; try{ stored=JSON.parse(localStorage.getItem(PETS_KEY)||'[]') }catch{}
  const list = stored.length>0 ? stored : basePets
  return list.map(p=>{ const c=getPhaseAndAdviceFromDOB(p.especie,p.dob,p.peso||0,p.raza||''); return {...p, edad:+c.ageYears.toFixed(1), fase:c.phase, consejoIA:c.advice, pesoEstado:c.weightStatus, pesoConsejo:c.weightAdvice, sensores:p.sensores||[], historial:p.historial||[], historialClinico:p.historialClinico||[], vacunas:p.vacunas||[], tratamientos:p.tratamientos||[], archivos:p.archivos||[] } })
}
...
export function getPetsWithDevices(){ const devs=loadDevices(), pets=loadPets(), map=new Map(); devs.forEach(d=>{ const arr=map.get(d.petId)||[]; arr.push(d); map.set(d.petId,arr) }); return pets.filter(p=>map.has(p.id)).map(p=>({...p,dispositivos:map.get(p.id)})) }

const baseCalendar=[ {id:'c1',date:'2025-10-18',type:'vacuna',note:'Vacuna rabia (Mia)'}, {id:'c2',date:'2025-10-25',type:'desparasitacion',note:'Desparasitación (Rocco)'}, {id:'c3',date:'2025-10-28',type:'acuario',note:'Cambio de agua (Nemo)'} ]
...
export function makeAdvice(metric,value,species){ if(metric==='hum'&&value<45) return 'Humedad baja.'; if(metric==='hum'&&value>70) return 'Humedad alta.'; if(metric==='temp'&&value<24&&species==='Reptil') return 'Temperatura baja.'; if(metric==='ph'&&value<6.5) return 'pH bajo.'; if(metric==='ph'&&value>7.8) return 'pH alto.'; if(metric==='pulso'&&value>140) return 'Pulso elevado.'; return 'Valores normales.' }

export function addClinicalNote(petId, note){ const pets=loadPets(); const updated=pets.map(p=>{ if(p.id!==petId) return p; const list=[...(p.historialClinico||[])]; list.unshift({...note}); return {...p, historialClinico:list} }); savePets(updated) }
export function addVaccine(petId, item){ const pets=loadPets(); const updated=pets.map(p=>{ if(p.id!==petId) return p; const list=[...(p.vacunas||[])]; list.unshift({...item}); return {...p, vacunas:list} }); savePets(updated) }
export function addTreatment(petId, t){ const pets=loadPets(); const updated=pets.map(p=>{ if(p.id!==petId) return p; const list=[...(p.tratamientos||[])]; list.unshift({...t}); return {...p, tratamientos:list} }); savePets(updated) }
export function addAttachment(petId, a){ const pets=loadPets(); const updated=pets.map(p=>{ if(p.id!==petId) return p; const list=[...(p.archivos||[])]; list.unshift({...a}); return {...p, archivos:list} }); savePets(updated) }
