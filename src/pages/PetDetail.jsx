...
import MedicalTabs from '../components/MedicalTabs.jsx'
  ...
  return (
    <Page title={`Perfil: ${pet.nombre}`}>
      <div className="flex flex-wrap gap-2 justify-end mb-4">
        <button onClick={() => setOpenEdit(true)} className="px-3 py-2 rounded-xl border hover:bg-emerald-50 flex items-center gap-1 text-sm"><Pencil size={16}/> Editar</button>
        <button onClick={deletePet} className="px-3 py-2 rounded-xl border border-red-300 text-red-600 hover:bg-red-50 flex items-center gap-1 text-sm"><Trash2 size={16}/> Borrar</button>
      </div>
      ...
      <div className="grid md:grid-cols-3 gap-4">
        <div className="bg-white rounded-2xl shadow-soft p-4 border border-emerald-100 md:col-span-2">
          ...
          <p className="text-sm text-gray-700 mt-1">
            {pet.especie}{pet.raza ? ` • ${pet.raza}` : ''} • {pet.edad?.toFixed?.(1)} años • {pet.fase}{pet.peso ? ` • ${pet.peso} kg` : ''}
          </p>
          <p className="text-sm text-emerald-700 mt-1">{pet.consejoIA}</p>
          {pet.pesoConsejo ? <p className="text-sm text-emerald-700">{pet.pesoConsejo}</p> : null}
        ...
        <div className="md:col-span-1">
          <div className="bg-white rounded-2xl shadow-soft p-4 border border-emerald-100">
            ...
            <p className="text-sm text-gray-700 mt-2">{makeAdvice('hum', data.hum ?? 52, pet.especie)}</p>
            <p className="text-sm text-gray-700 mt-1">{makeAdvice('ph', data.ph ?? 7.2, pet.especie)}</p>
          </div>
        </div>
      </div>
      ...
      <EditPetModal open={openEdit} pet={pet} onClose={() => setOpenEdit(false)} onSaved={() => {}} />
    </Page>
  )
}
