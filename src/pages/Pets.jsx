...
export default function Pets(){
  ...
  useEffect(()=>{
    const id=setInterval(()=>setPets(loadPets()),1200)
          ...
          <div key={p.id} className='bg-white rounded-2xl shadow-soft p-4 border border-emerald-100'>
            <Link to={`/mascotas/${p.id}`} className='block'>
              ...
              <p className='text-xs text-gray-500'>
                {p.especie} {p.raza?`• ${p.raza}`:''} • {p.edad?.toFixed?.(1)} años • {p.fase}
              </p>
              {p.peso
                ? <p className='text-xs text-gray-500'>Peso: {p.peso} kg — {p.pesoEstado}</p>
                : <p className='text-xs text-gray-400'>Peso no indicado</p>}
            ...
            <div className='mt-3 flex gap-2'>
              <button
                onClick={()=>{setActivePet(p); setOpenDev(true)}}
                className='px-3 py-2 text-sm rounded-xl border hover:bg-emerald-50'>
